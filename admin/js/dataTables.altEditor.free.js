...
(function ($, window, document, undefined) {
    ...
    var altEditor = function (dt, opts) {
		...
		this.customOptions ={};
		this.customOptions[1]={'1':'crawlspider.com','2':'mockup.com','3':'wordport.com'};
        /**
    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
				...
				//cscm_table_list["cscm_list_of_urls_tbl"]
                ...
                this.language.edit = { title: this.c.language.edit.title || 'Edit record',
                                       button: this.language.edit.button || 'Edit'
                                     };
                this.language.delete = this.language.delete || {};
                this.language.delete = { title: this.c.language.delete.title || 'Delete record',
                                         button: this.language.delete.button || 'Delete' };
                this.language.add = this.language.add || {};
                this.language.add = { title: this.c.language.add.title || 'Add record',
            ...
            */
			...
			isReadOnly: function(pColumnDef,pButtonClass)
			{
				
				if (!pColumnDef.readonly) return '';
				else
				{
					//if you came here then either readonly=true or readonly has buttonclass true/false
					if (pColumnDef.readonly[pButtonClass]===undefined) //check if buttonclass not defined
					{
						return 'readonly'; //readonly true for add and edit
					}
					else 
					{
						if (pColumnDef.readonly[pButtonClass]) return 'readonly';
						else return '';
					}
			...
			isHidden: function(pColumnDef,pButtonClass)
			{
				
				if (!pColumnDef.hidden) return false;
				else
				{
					//if you came here then either readonly=true or readonly has buttonclass true/false
					if (pColumnDef.hidden[pButtonClass]===undefined) //check if buttonclass not defined
					{
						return true; //readonly true for add and edit
					}
					else 
					{
						return pColumnDef.hidden[pButtonClass];
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                for (var j in columnDefs) {
                    ...
                    if ((columnDefs[j].type.indexOf("hidden") >= 0)|| this.isHidden(columnDefs[j],buttonClass)) {
                        ...
                        // Adding select-fields
							...
							if (typeof(this.customOptions[j]) != 'undefined')
							{
								optionsArray=this.customOptions[j];
							}
                            ...
                            data += "<select class='form-control" + (columnDefs[j].select2 ? ' select2' : '')
                                ...
                                + "' "
                                + (this.isReadOnly(columnDefs[j],buttonClass))
